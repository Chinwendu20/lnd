...
const (
	userInitiatedType tlv.Type = 0

	linkFailureErrorType tlv.Type = 1

	htlcActionsType tlv.Type = 2

	SignatureType tlv.Type = 3
)
...
func HtlcActionEncoder(w io.Writer, val interface{}, buf *[8]byte) error {
	if v, ok := val.(*map[string][]HTLC); ok {
		...
		for action, htlcs := range *v {
			...
			err = actionStream.Encode(&actionBytes)
			if err != nil {
				return err
			}
			if err != nil {
...
func HtlcActionDecoder(r io.Reader, val interface{}, buf *[8]byte,
	...
	if v, ok := val.(*map[string][]HTLC); ok {
		...
		for i := uint64(0); i < numRecords; i++ {
			...
			err = actionStream.Decode(&actionTlvReader)
			if err != nil {
				return err
			}
...
func SerializeLocalCloseInfo(w io.Writer, lc *LocalForceCloseInfo) error {
	linkFailureErrorByte := []byte(lc.LinkFailureError)

	tlvStream, err := tlv.NewStream(
		tlv.MakePrimitiveRecord(
			userInitiatedType, &lc.UserInitiated,
		),
		tlv.MakePrimitiveRecord(
			linkFailureErrorType,
			&linkFailureErrorByte,
		),
		tlv.MakeDynamicRecord(
			htlcActionsType, &lc.HtlcActions,
			HtlcActionRecordSize(
				&lc.HtlcActions),
			HtlcActionEncoder,
			HtlcActionDecoder,
		),
...
func DeserializeLocalCloseInfo(r io.Reader) (LocalForceCloseInfo, error) {

	var (
		userInitiated    bool
		linkFailureError string
		lc               LocalForceCloseInfo
	)

	lc.HtlcActions = make(map[string][]HTLC)
	linkFailureErrorByte := []byte(linkFailureError)

	tlvStream, err := tlv.NewStream(
		tlv.MakePrimitiveRecord(
			userInitiatedType, &userInitiated,
		),
		tlv.MakePrimitiveRecord(
			linkFailureErrorType,
			&linkFailureErrorByte,
	...
	lc.UserInitiated = userInitiated
	lc.LinkFailureError = linkFailureError
